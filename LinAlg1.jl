...
md""" # Linear Algebra Day 1
**Vector Spaces, Linearity, Inner Products**
"""
...
begin 
	mutable struct Neuron
		...
		λ₀
		spikes
		Neuron(id, v₀, λ₀, spikes) = new(id, normalize(v₀), λ₀, spikes)
	end
	...
	function Population(n::Number, λ₀)
	    neurons=[Neuron(i, rand(Float64, (1,3))*2 .- 1, λ₀, []) for i ∈ 1:n]
		Population(neurons)
	end
end
...
begin
	function stimulate!(N::Neuron, v, t)
		l=last(t)-first(t)
		v=normalize(v)
		numspk=rand(Poisson((dot(N.v₀, v)+1)*N.λ₀/2.0*l))
		N.spikes = sort(sample(t, numspk; replace=false))
	end

	function stimulate!(P::Population, v, t)
		stimulate!.(P.neurons, Ref(v), Ref(t))
	end
...
function spikerate(N; center=false, norm=false)
	...
	λ=1/mean(N.spikes[2:end].-N.spikes[1:end-1])
	if center
		λ=λ-N.λ₀/2
	end
	if norm
...
begin
	...
	function showbasis(P)
		b=x->mke.Vec3f(x.v₀[1], x.v₀[2], x.v₀[3])
		nb = b.(P.neurons)
		# b=x->x.v₀
		# nb = vcat(b.(P.neurons)...)
		cset=palette(:tab10)
		origin=fill(mke.Point3f(0,0,0), length(P.neurons))
		f = mke.Figure(resolution = (1000, 1000))
		s = mke.LScene(f[1,1])

	
		mke.arrows!(s, origin, nb, arrowsize=.125, linewidth=.05, color=cset[1:length(nb)])
	...
	function showvec(v)
		v=normalize(v)
		vec=mke.Vec3f(v[1], v[2], v[3])
		
		# b=x->x.v₀
		# nb = vcat(b.(P.neurons)...)
		origin=mke.Point3f(0.0, 0.0,0.0)
		f2 = mke.Figure(resolution = (1000, 1000))
		s2 = mke.Axis3(f2[1,1]; limits=(-1,1,-1,1,-1,1))

		mke.arrows!(s2, [origin], [vec], arrowsize=.125, linewidth=.05, color=:red)
